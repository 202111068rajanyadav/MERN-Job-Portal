...
const MyJobs = () => {
  ...
  useEffect(() => {
    setIsLoading(true);
    fetch(`http://localhost:5000/myJobs/abc@gmail.com`)
      .then((res) => res.json())
      .then((data) => {
...
{
  const newJobs=jobs.filter((item=>jobs.indexOf(item)!=index))
  setJobs(newJobs)
}
      ...
      <div className="my-jobs-container">
        ...
        <div  className="search-box p-2 text-center mb-2">
          <input
            ...
            name="search"
            id="search"
            className="py-2 pl-3 border focus:outline-none lg:w-6/
        12 mb-4 w-2/5 "
          />
          <button
    ...
    <div className="rounded-t mb-0 px-4 py-3 border-0">
      <div className="flex flex-wrap items-center">
        ...
        <div className="relative w-full px-4 max-w-full flex-grow flex-1 text-right">
          ...
          type="button" ><a href="/post-job">Post job</a></button>
        </div>
      </div> 
    </div>
      ...
      <table className="items-center bg-transparent w-full border-collapse ">
        ...
        <tbody>
          {jobs.map((items)=>{
            ...
            <td className="border-t-0 px-6 align-middle border-l-0 border-r-0 text-xs whitespace-nowrap p-4" onClick={
              ()=>{
                deleteItem(jobs.indexOf(items));
